...
/*
 * ARMv6 page table entries for ARM11 MPcore:
 * - use TEX=001 to set write-back/write-allocate
 * - use shared access (set S bit)
 * - user mappings use ASID (set nG bit)
 */
const struct armv_pte	armv_pte_v6mp = {
...
__SRCVERSION( "$URL: http://svn/product/branches/6.5.0/trunk/hardware/startup/lib/arm/armv_pte_v6mp.c $ $Rev: 217585 $" );
