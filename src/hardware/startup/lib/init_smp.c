		...
		if(num_cpus > 1) {
			...
			// For backwards compatability, remove later
			{
				...
				for(i = 0; i < num_cpus; ++i) {
					...
					/*  Because the lapicid_to_index array used below has a hard coded size of 16,
					 *  defined in x86/syspage.h, if there are more than 16 CPUs memory will get
					 *  corrupted when initializing the array. Instead of changing syspage.h,
					 *  which would have far reaching effects we skip over the population of the
					 *  lapicid_to_index array when iterating CPU id's greater than 15
					 */
					if (i >= NUM_ELTS(x86_smp->lapicid_to_index)) {
						kprintf("lapicid_to_index init skipped for CPU's %u to %u\n", i, num_cpus);
						break;
					}
					 ...
					 */
					x86_smp->lapicid_to_index[i] = cpuid_remap[i];
					if (debug_flag > 1) kprintf("CPU%u has APIC ID 0x%x\n", i, x86_smp->lapicid_to_index[i]);
				}
			}
#endif
		}
...
__SRCVERSION("init_smp.c $Rev: 687353 $");
