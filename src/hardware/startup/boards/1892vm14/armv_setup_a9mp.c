...
void armv_setup_a9mp(struct cpuinfo_entry *cpu, unsigned cpuid)
{
	unsigned aux;
	unsigned cpunum;
	...
	if (cpunum == 0) {
		 ...
		 */
		ctrl = in32(mpcore_scu_base + MPCORE_SCU_CTRL);
		ctrl &= ~MPCORE_SCU_CTRL_EN;
		out32(mpcore_scu_base + MPCORE_SCU_CTRL, ctrl);

		out32(mpcore_scu_base + MPCORE_SCU_INVALIDATE, -1);
		out32(mpcore_scu_base + MPCORE_SCU_CTRL, ctrl | MPCORE_SCU_CTRL_EN);
	} else {
		/*
