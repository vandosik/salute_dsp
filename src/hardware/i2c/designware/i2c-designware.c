...
i2c_status_t designware_i2c_send( void *handler, void *buf, uint32_t len, uint32_t stop )
{
    ...
    /* Send data */
    if ( designware_i2c_write( dev, dev->slave_addr.addr, 0 /* offset */, 1 /* offset length */, (uint8_t *)buf, len ) )
    {
...
i2c_status_t designware_i2c_recv( void *handler, void *buf, uint32_t len, uint32_t stop )
{
    ...
    /* Receive data */
    if ( designware_i2c_read( dev, dev->slave_addr.addr, 0 /* offset */, 1 /* offset length */, (uint8_t *)buf, len ) )
    {
