...
static uint32_t swap_be32(uint32_t data)
{
	return ENDIAN_BE32(data);
}
...
//		fprintf(stderr, "length sanity check: len %d bch->n %d ecc_bits %d\n", 8*len, bch->n, bch->ecc_bits);
		return -EINVAL;
	}
	...
	if (!syn) {
		if (!calc_ecc) {
			...
			if (!data || !recv_ecc) {
//				fprintf(stderr, "decode_bch: bad buffs\n");
				return -EINVAL;
			}
...
//	fprintf(stderr, "decode_bch: error %d\n", err );
