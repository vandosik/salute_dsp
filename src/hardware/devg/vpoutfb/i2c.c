...
int i2c_read( vpout_context_t *vpout, vpout_draw_context_t *vpout_draw, uint8_t port, uint8_t addr, unsigned short offset, uint8_t *buf, unsigned count )
{
    ...
    int             rbytes;
    uint32_t        speed    = vpout->hdmi[port].device.it66121.speed * 1000;

    if ( bus_en[port] != 1 )
    {
        disp_printf( adapter, "[vpout: i2c] Error: I2C bus read failed [bus not found: bus=#%u, port=%d]",
                     vpout->hdmi[port].device.it66121.bus, port );
        return (-1);
    }

    if ( devctl( bus[port], DCMD_I2C_SET_BUS_SPEED, &speed, sizeof( speed ), NULL ) )
    {
        disp_printf( adapter, "[vpout: i2c] Error: I2C bus read failed [can't set bus speed: bus=#%u, port=%d]",
                     vpout->hdmi[port].device.it66121.bus, port );
        return (-1);
    }
    ...
    //~ if ( status )
    //~ {
        //~ disp_printf( adapter, "[vpout: i2c] Error: I2C bus read failed [transaction failed on write address: bus=#%u, addr=0x%x, port=%d]",
                     //~ vpout->hdmi[port].device.it66121.bus, addr, port );
        //~ return (-1);
    //~ }
    ...
    if ( status )
    {
        disp_printf( adapter, "[vpout: i2c] Error: I2C bus read failed [transaction failed on read: bus=#%u, addr=0x%x, port=%d]",
                     vpout->hdmi[port].device.it66121.bus, addr, port );
        return (-1);
    }
...
int i2c_write( vpout_context_t *vpout, vpout_draw_context_t *vpout_draw, uint8_t port, uint8_t addr, unsigned short offset, uint8_t *buf, unsigned count )
{
    ...
    if ( bus_en[port] != 1 )
    {
        disp_printf( adapter, "[vpout: i2c] Error: I2C bus writing failed [bus not found: bus=#%u, port=%d]",
                     vpout->hdmi[port].device.it66121.bus, port );
        return (-1);
    }
    ...
    if ( status )
    {
        disp_printf( adapter, "[vpout: i2c] Error: I2C bus write failed [transaction failed on write: bus=#%u, addr=0x%x, port=%d]",
                     vpout->hdmi[port].device.it66121.bus, addr, port );
        return (-1);
    }
