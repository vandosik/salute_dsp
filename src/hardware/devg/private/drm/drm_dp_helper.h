...
static inline int
drm_dp_max_link_rate(const uint8_t dpcd[DP_RECEIVER_CAP_SIZE])
{
    return drm_dp_bw_code_to_link_rate(dpcd[DP_MAX_LINK_RATE]);
}

static inline uint8_t
drm_dp_max_lane_count(const uint8_t dpcd[DP_RECEIVER_CAP_SIZE])
{
    return dpcd[DP_MAX_LANE_COUNT] & DP_MAX_LANE_COUNT_MASK;
}

static inline bool
drm_dp_enhanced_frame_cap(const uint8_t dpcd[DP_RECEIVER_CAP_SIZE])
{
