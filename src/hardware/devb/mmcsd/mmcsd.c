...
int mmc_switch( SIM_HBA *hba, int flgs, uint32_t cmdset, uint32_t mode, uint32_t index, uint32_t value )
{
	...
	if( ( status = mmc_sendcmd( hba, MMC_SWITCH, MMC_RSP_R1B, MMC_CMD_INTR, ( mode << 24 ) | ( index << 16 ) | ( value << 8 ) | cmdset ) ) == MMC_SUCCESS && flgs ) {
		status = mmc_wait_command( hba, MMC_TIME_DEFAULT);
	}
	return( status );
}
...
int mmc_get_card_status( SIM_HBA *hba )
{
	...
	return( mmc_wait_command( hba, MMC_TIME_DEFAULT ) );
}
...
static int mmc_ident_read_ext_csd(SIM_HBA *hba, int intr)
{
	...
	card_type_v4 = ext_csd[EXT_CSD_CARD_TYPE];
	if (card_type_v4 & EXT_CSD_CARD_TYPE_52_CAPABLE)
	{
		ext->csd.mmc_csd.ext_csd.hs_max_dtr = 52000000;
	}
	else if (card_type_v4 & EXT_CSD_CARD_TYPE_26_CAPABLE)
	{
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/mmcsd/mmcsd.c $ $Rev: 809661 $")
#endif
