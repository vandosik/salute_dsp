...
static int _sdhci_setup_dma(SIM_HBA *hba, paddr_t paddr, int len, int dir)
{
	...
	if ( sdhci->cap & SDHCI_CAP_DMA ) {
		...
		// Use ADMA2
		int i;
		sdhci_adma32_t *adma = (sdhci_adma32_t *)sdhci->tbuf;
		for ( i=0; i<blkcnt; ++i ) {
			adma->addr = paddr;
			adma->attr = SDHCI_ADMA2_VALID | SDHCI_ADMA2_TRAN;
			adma->len = sdhci->blksz;
			paddr += sdhci->blksz;
			adma++;
		}
		adma--;
		...
		SDHCI_HOSTCTL(base) =
			(SDHCI_HOSTCTL(base) & ~SDHCI_HOSTCTL_DMAMASK) |
			SDHCI_HOSTCTL_ADMA32;
		SDHCI_BLKCNT(base) = blkcnt;
		SDHCI_XFRMODE(base) = xmode;
...
static int _sdhci_setup_pio(SIM_HBA *hba, int len, int dir)
{
	...
	if (dir == MMC_DIR_IN)
 	...
 	if (sdhci->cap & SDHCI_CAP_DMA) {
		...
		int i;
		sdhci_adma32_t *adma =
			(sdhci_adma32_t *)(sdhci->tbuf + SDHCI_DMABUF_PIODSC);
		paddr_t paddr = sdhci->tbuf_phy;
		for ( i=0; i<blkcnt; ++i ) {
			adma->addr = paddr;
			adma->attr = SDHCI_ADMA2_VALID | SDHCI_ADMA2_TRAN;
			adma->len = sdhci->blksz;
		...
		SDHCI_BLKCNT(base) = blkcnt;
		SDHCI_XFRMODE(base) = xmode;
		SDHCI_ADMA_ADDRL(base) = sdhci->tbuf_phy + SDHCI_DMABUF_PIODSC;
	}
	sdhci->pio_enable = 1;
...
static int _sdhci_clock(SIM_HBA *hba, int *clock, int high_speed)
{
	...
	if ((SDHCI_CTRLRVER(base) & 0xf) >= SDHCI_SD_VER_3) {
		/* Version 3.00 divisors */
		if (clk <= *clock)
			div = 1;
		else {
	...
	clkctl = (div & 0xFF) << SDHCI_CLKCTL_DIV_SHIFT;
	clkctl |= ((div & 0x300) >> SDHCI_CLKCTL_DIV_SHIFT) << 6;
	SDHCI_CLKCTL(base)  = clkctl | SDHCI_CLKCTL_ICE;
	SDHCI_CLKCTL(base)  = clkctl | SDHCI_CLKCTL_ICE | SDHCI_CLKCTL_CLKEN;
	SDHCI_HOSTCTL(base) = hostctl;
...
int	sdhci_init(SIM_HBA *hba)
{
...
#ifdef SDHCI_ADMA_SUP
	if (cap & (SDHCI_CAP_DMA | SDHCI_CAP_ADMA2))
	...
	if ( !(sdhci->cap & SDHCI_CAP_DMA) && !(sdhci->cap & SDHCI_CAP_ADMA2) ) {
		fprintf( stderr, "This controller don't support SDMA and ADMA2 mode\n" );
		goto fail3;
	}
	...
	if (!cfg->Description[0])
		strncpy((char *)cfg->Description, "Generic SDHCI", sizeof(cfg->Description));
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/mmcsd/chipsets/sim_sdhci.c $ $Rev: 735969 $")
#endif
