...
int mmc_wait_command( SIM_HBA *hba, int timeo )
{
	SIM_MMC_EXT		*ext;
	struct _pulse	pulse;
	iov_t			iov;
	...
	while( 1 ) {
		...
		switch( pulse.code ) {
			...
			case SIM_TIMER:
					// check media if card detection interrupt is not supported
...
#ifdef MMCSD_DEBUG
		mmc_slogf( hba, _SLOGC_SIM_MMC, _SLOG_ERROR, 1, "%s: intr %x", __FUNCTION__, intr );
#endif
		...
		if( ( intr & ( MMC_INTR_COMMAND | MMC_INTR_DATA | MMC_INTR_RBRDY | MMC_INTR_WBRDY ) ) ) {
			return( MMC_SUCCESS );
		}
		
		if( timeo != MMC_TIME_INFINITY ) {
			clock_gettime( CLOCK_MONOTONIC, &time );
			if( timeout < time.tv_sec ) {
#ifdef MMCSD_DEBUG
...
int mmc_wait_card_ready(SIM_HBA *hba)
{
	...
	clock_gettime( CLOCK_MONOTONIC, &time );
	timeout = time.tv_sec + (ext->busy_retry/1000);
	while(1){
...
static void mmc_dump_vuc(SIM_MMC_EXT *ext, MMCSD_VUC_CMD *vuc)
{
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc flags: %x", vuc->flags);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc arg: %x", vuc->arg);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc cmd: %04x", vuc->opcode);
	if(vuc->flags & MMCSD_VUC_RCA){
		slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc rca: %04x", ext->rca);
	}
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc blksz: %d", vuc->blk_sz);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc paddr: %x", vuc->data_ptr);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc data_len: %d", vuc->data_len);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc buf_off: %d", vuc->buf_off);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc timeout: %d", vuc->timeout);
	slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "vuc delay: %x", vuc->postdelay_us);
}

static inline void mmc_restore_ac12(SIM_MMC_EXT	*ext, uint32_t ac12)
{
	ext->hccap &= ~MMC_HCCAP_ACMD12;
	ext->hccap |= ac12;
}

static int mmc_vuc_cmd( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
	SIM_MMC_EXT		*ext;
	MMCSD_VUC_CMD	*vuc;
	int				status;
	...
	if (!(ext->eflags & MMC_EFLAG_READY)) {
		mmc_sense(hba, SK_NOT_RDY, ASC_MEDIA_NOT_PRESENT, 0);
		ccb->cam_devctl_status = ENODEV;
		return( CAM_REQ_CMP_ERR );
	}

	// Save the ACMD12 flag
	ac12 = ext->hccap & MMC_HCCAP_ACMD12;
	if (vuc->flags & MMCSD_VUC_NOAC12)
		ext->hccap &= ~MMC_HCCAP_ACMD12;
	else
		ext->hccap |= MMC_HCCAP_ACMD12;
	...
	for(; ;){
		...
		if(((vuc->flags & MMCSD_VUC_DATA_PHYS) && ((ext->eflags & MMC_EFLAG_PIO) || ext->setup_dma==NULL))
			|| (!(vuc->flags & MMCSD_VUC_DATA_PHYS) && (ext->setup_pio==NULL))){
			slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "%s: %s not supported", __FUNCTION__, (vuc->flags & MMCSD_VUC_DATA_PHYS)?"DMA":"PIO");
			ccb->cam_devctl_status = EINVAL;
			mmc_dump_vuc(ext, vuc);
			mmc_restore_ac12(ext, ac12);
			return CAM_REQ_CMP_ERR;
		}

		vuc->result = MMC_VUC_SUCCESS;
		if(vuc->flags & (MMC_RSP_PRESENT<<MMCSD_VUC_RESP_OFF)){
			memset((void *)&vuc->resp[0], 0, 4*sizeof(uint32_t));
		}

		//do we need command 55?
		if(vuc->flags & MMCSD_VUC_ACMD){
			if( ( status = mmc_sendcmd( hba, MMC_APP_CMD, MMC_RSP_R1, MMC_CMD_INTR, ext->rca << 16) ) == MMC_SUCCESS ) {
				status = mmc_wait_command( hba, (vuc->timeout) ? vuc->timeout : MMC_TIME_DEFAULT );
			}

			if(status != MMC_SUCCESS){
				ccb->cam_devctl_status = EIO;
				slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "%s(%d): status (%x)", __FUNCTION__, __LINE__, status);
				mmc_dump_vuc(ext, vuc);
				mmc_restore_ac12(ext, ac12);
				return CAM_REQ_CMP_ERR;
			}
		...
		if(vuc->flags & MMCSD_VUC_DATA_MSK){
			...
			if(vuc->flags & MMCSD_VUC_DATA_IN){
				ext->dir = MMC_DIR_IN;
				flags |= MMC_CMD_DATA_IN;
			}else{
				ext->dir = MMC_DIR_OUT;
			}
			if(vuc->flags & MMCSD_VUC_DATA_PHYS){
				flags |= MMC_CMD_DATA_DMA;
				nbytes = ext->setup_dma(hba, vuc->data_ptr, vuc->data_len, ext->dir);
			}else{
				nbytes = ext->setup_pio(hba, vuc->data_len, ext->dir);
			}
			if (nbytes < vuc->data_len){
				ccb->cam_devctl_status = EINVAL;
				slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "%s(%d): status (%x)", __FUNCTION__, __LINE__, status);
				mmc_dump_vuc(ext, vuc);
				mmc_restore_ac12(ext, ac12);
		...
		if( ( status = mmc_sendcmd( hba, vuc->opcode, (vuc->flags & MMCSD_VUC_RESP_MSK)>>MMCSD_VUC_RESP_OFF, flags, arg) ) == MMC_SUCCESS ) {
			status = mmc_wait_command( hba, (vuc->timeout) ? vuc->timeout : MMC_TIME_DEFAULT );
		}

		if(vuc->flags & (MMC_RSP_PRESENT<<MMCSD_VUC_RESP_OFF)){
			memcpy((void *)&vuc->resp[0], (void *)&ext->mmc_resp[0], 4*sizeof(uint32_t));
		}
		...
		if(vuc->flags & MMCSD_VUC_DATA_MSK){
			if( flags & MMC_CMD_DATA_DMA){
				...
				for(;;) {
					do{
						nbytes = ext->pio_done(hba, (char *)buf + vuc->buf_off + ext->offset,
									vuc->data_len - ext->offset, ext->dir);
						ext->offset += nbytes;
					}while(nbytes > 0 && ext->offset != vuc->data_len);

					if (ext->offset != vuc->data_len){
						if((status = mmc_wait_command( hba, (vuc->timeout) ? vuc->timeout : MMC_TIME_DEFAULT )) != MMC_SUCCESS){
							vuc->result = MMC_VUC_FAILED;
							slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "%s(%d): status (%x)", __FUNCTION__, __LINE__, status);
							mmc_dump_vuc(ext, vuc);
							mmc_restore_ac12(ext, ac12);
						...
						if(vuc->flags & MMCSD_VUC_DATA_OUT){
							mmc_wait_command( hba, (vuc->timeout) ? vuc->timeout : MMC_TIME_DEFAULT );
						}
						break;
					}
				}
...
static void mmc_devctl(SIM_HBA *hba)
{
	...
	switch (ccb->cam_devctl_dcmd) {
		...
		case DCMD_MMCSD_VUC_CMD:
			ccb->cam_ch.cam_status = mmc_vuc_cmd( hba, ccb );
			break;
...
static void mmc_ident_irq(SIM_HBA *hba, int intr)
{
	...
	if (mmc_card_ident(hba, intr) == MMC_SUCCESS) {
		if (ext->ident == MMCSD_IDENT_NONE) {	// ident complete
			...
			atomic_clr_value( &ext->eflags, MMC_EFLAG_RESETTING );
			if(!(ext->eflags & MMC_EFLAG_READY)){
		...
		if( ext->eflags & MMC_EFLAG_RESETTING ) {	// failure trying to power up/ident, retry via pulse handler
	...
	if (ext->stop == MMCSD_STOP_ISSUED) {			// stop command issued ?
		if (intr & MMC_INTR_ERROR) {				// error ?
			...
			if(stops_retry++>5 || (mmc_stop_xfer(hba) != MMC_SUCCESS)){
				// try to reset/reinit, we set ident state so
				// we don't try to start a new request
...
static int mmc_process_interrupt(SIM_HBA *hba, int irq)
{
	...
	if (intr & MMC_INTR_CARD) {
		...
		case MMCSD_STATE_DATA:
			...
			if(ccb && ccb->cam_ch.cam_status != CAM_REQ_INPROG){
				if((ext->eflags & MMC_EFLAG_AC13)
					&& ccb && (ccb->cam_ch.cam_status == CAM_REQ_CMP)
					&& (ccb->cam_cdb_io.cam_cdb_bytes[0] == SC_READ10)
					&& (ext->nbytes == ext->mmc_blksize)) {

					if (irq == hba->cfg.IRQRegisters[0])
						InterruptUnmask(hba->cfg.IRQRegisters[0], hba->iid);
				...
				/*
				 * FIXME: mmc_wait_card_ready() assumes the hba->cfg.IRQRegisters[0] is unmasked,
				 * but this could not be always be true.
				 */
				if(mmc_wait_card_ready(hba)!=MMC_SUCCESS){
					mmc_error(hba, CAM_PROVIDE_FAIL);
				}
	...
	while (1) {
		...
		switch (pulse.code) {
			...
			case SIM_TIMER:
				...
				ccb = ext->nexus;
				if (ccb && !(ext->eflags & MMC_EFLAG_INSERTED)
						&& ((ccb->cam_ch.cam_status == CAM_REQ_INPROG) || (ccb->cam_ch.cam_status == CAM_REQ_CMP_ERR))) {
					mmc_error(hba, MMC_NOT_PRESENT);
					ext->nexus = NULL;
					ext->state = MMCSD_STATE_IDLE;
					simq_post_ccb(hba->simq, ccb);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/mmcsd/sim_mmc.c $ $Rev: 796468 $")
#endif
