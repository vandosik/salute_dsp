...
const MODULE_ENTRY sim_module = {
	"sdmmc",
	&sdmmc_sim_args,		// called once for every time "sdmmc" module name is on the cmd line
...
/* sdmmc_alloc_hba
 * Initialization of single hba structure
 */
SIM_HBA *sdmmc_alloc_hba( )
{
	...
	/* sim_alloc_hba is libcam function */
...
void sdmmc_display_ccb( SIM_HBA *hba, CCB *ccb_ptr )
{
	CCB_SCSIIO	*ccb;
	uint32_t	i;
	char		buf[50];
	...
	for( ccb = ccb_ptr; ccb; ccb = (((CCB_HEADER *)ccb_ptr)->cam_func_code == XPT_SCSI_IO &&
		...
		switch( ((CCB_HEADER *)ccb_ptr)->cam_func_code ) {
			case XPT_SCSI_IO: {
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "MMC XPT_SCSI_IO     " );
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "addr=%p len=%d stat=%d path=%d targ=%d lun=%d flag=%08lx timeout=%ld vuflag=%04lx",
						ccb->cam_ch.my_addr,
						...
						ccb->cam_timeout,
						ccb->cam_vu_flags );
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "     pdrv=%p next=%p req_map=%p cbfcnp=%p xfer_len=%ld"
				...
				if( ccb->cam_ch.cam_flags & CAM_SCATTER_VALID ) {
					...
					cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 5, "num_sg=%d\n", ccb->cam_sglist_cnt);
					for( i = 0; i < ccb->cam_sglist_cnt; i++, sg++ ) {
						cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 5, "          sgentry %d) addr=%08x count=%ld",
							i + 1, sg->cam_sg_address, sg->cam_sg_count );
					}
				}
				else {
					cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 5, "data_ptr=%08x", ccb->cam_data.cam_data_ptr );
				}
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "     cdblen=%d  (", ccb->cam_cdb_len );
				...
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, " %s )", buf );
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, " )" );
				if( (ccb->cam_ch.cam_flags & CAM_CDB_LINKED) && ccb->cam_next_ccb ) {
					cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "LINKED==>" );
				}
				break;
			}
			...
			case XPT_DEVCTL: {
				...
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "MMC type=%x addr=%p len=%d stat=%d path=%d targ=%d lun=%d flag=%08lx, dcmd=%x, dsize=%d",
						ccb->cam_func_code,
						ccb->my_addr,
						ccb->cam_ccb_len,
						ccb->cam_status,
			...
			default: {
				CCB_HEADER *ccb = ccb_ptr;
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 4, "MMC type=%d addr=%p len=%d stat=%d path=%d targ=%d lun=%d flag=%08lx",
						ccb->cam_func_code,
						ccb->my_addr,
						ccb->cam_ccb_len,
						ccb->cam_status,
...
//	cam_slogf( _SLOGC_SIM_MMC, _SLOG_DEBUG1, hba->verbosity, 1, "%s:  hba %p", __FUNCTION__, hba );
...
int sdmmc_reg( SIM_HBA *hba )
{
	...
	cam_slogf( _SLOGC_SIM_MMC, _SLOG_DEBUG1, hba->verbosity, 1, "%s: start", __FUNCTION__ );
	...
	if( sdmmc_create_thread( &hba->tid, &attr, sdmmc_driver_thread, hba, ext->priority, &hba->state, "sdmmc_driver_thread" ) != EOK ) {
	...
	if(hba->pathid == -1) {
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: xpt_bus_register failure\n", __FUNCTION__);
		return (CAM_FAILURE);
	} else {
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_DEBUG1, hba->verbosity, 1, "%s: xpt_bus_register success with pathid = %d", __FUNCTION__, hba->pathid);
	}
...
int sdmmc_attach( SIM_HBA *hba, struct sdio_connection *connection, sdio_device_instance_t *instance )
{
	...
	// XXX we have to be here when hba->device.flags are already set by _sdio_attach() function !!!
	...
	if( sdio_attach( connection, instance, &ext->device, hba ) == EOK ) {
		...
		if( ( ext->dev_inf.caps & DEV_CAP_ASSD ) ) {	//  Advanced Security SD Extension Capability
			sdmmc_assd_init( hba );
		}

		status = sdmmc_reg( hba );
	} else {
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, hba->verbosity, 1, "%s: sdio_attach FAILURE", __FUNCTION__ );
	}
...
int sdmmc_error( SIM_HBA *hba, CCB_SCSIIO *ccb, int status )
{
	...
	switch( status ) {
		...
		case ENODEV:
			atomic_clr( &ext->eflags, SDMMC_EFLAG_PRESENT );
//			status = sdmmc_sense( ccb, SK_NOT_RDY, ASC_MEDIA_NOT_PRESENT, 0 ); break;
...
int sdmmc_msense( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
	SIM_SDMMC_EXT		*ext;
...
int sdmmc_std_inquiry( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
	...
	iptr->rmb			= ( ext->hc_inf.caps & HC_CAP_SLOT_TYPE_EMBEDDED ) ? CAM_FALSE : CAM_TRUE;	// Is media removable?
	iptr->version		= INQ_VER_SPC3;		// SPC-3
	iptr->adlen			= 96 - 5;	// nbytes after adlen field

	strcpy( (char *)iptr->vend_id, "SDMMC:" );
	strcpy( (char *)iptr->prod_id, (char *)ext->dev_inf.pnm );
	...
	if( ccb->cam_dxfer_len > 56 ) {
		strcpy( (char *)iptr->vend_spc, ext->targets[ccb->cam_ch.cam_target_id].partitions[ccb->cam_ch.cam_target_lun].name );
	}
...
int sdmmc_sync( SIM_HBA *hba, CCB_SCSIIO *ccb )
{
	...
	if( ( status = sdmmc_unit_ready( hba, ccb ) ) == CAM_REQ_CMP ) {
		if( ( status = sdio_flush_cache( ext->device ) ) != EOK ) {
			status = sdmmc_error( hba, ccb, status );
		}
	}
	return( status ? status : CAM_REQ_CMP );
...
#if 0
{
	...
	switch( op ) {
		...
		case PM_SLEEP:
			sdmmc_timer_settime( ext->pm_timerid, 0, CAM_FALSE );
			sdio_pwrmgnt( ext->device, PM_SLEEP );
			break;
...
int sdmmc_write_protect( SIM_HBA *hba, int op, int partition, int mode, uint32_t lba, uint32_t nlba, uint64_t *prot )
{
	...
	switch( op ) {
		...
		case MMC_SEND_WRITE_PROT_TYPE:
			...
			sge.sg_count = sz; sge.sg_address = (paddr_t)pdata;
...
int sdmmc_rw( SIM_HBA *hba, SDMMC_PARTITION *part, int flgs, uint32_t addr, int dlen, sdio_sge_t *sgl, int sgc, void *mhdl, uint32_t timeout )
{
	...
	if( status ) {
		...
		//	Update device info because device capabilities may have been
		//	changed by sdio_reset() or sdio_bus_error().
		sdio_dev_info(dev, di);
		...
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  flgs 0x%x, len %d, status 0x%x, cstatus 0x%x, rsp[0] 0x%x",
			__FUNCTION__, flgs, dlen, status, cstatus, rsp[0] );
	}
...
//				if( ( wptr = mmap( NULL, sgp->sg_count, PROT_READ | PROT_WRITE | PROT_NOCACHE, MAP_ANON | MAP_PHYS, -1, sgp->sg_address ) ) == MAP_FAILED ) {
...
void *sdmmc_driver_thread( void *hdl )
{
	...
	if( stat ) {
		sdmmc_set_thread_state( &hba->state, CAM_TSTATE_INIT_FAILURE );
		return( NULL );
	}

	sdmmc_set_thread_state( &hba->state, CAM_TSTATE_INITIALIZED );
...
long sdmmc_sim_init( SIM_HBA *hba, long path )
{
	hba = hba, path = path;
...
int sdmmc_sim_detach( )
{
	...
	cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: driver is terminating...", __FUNCTION__ );
...
int sdmmc_path_inq( SIM_HBA *hba, CCB_PATHINQ *ccb )
{
	...
	strcpy( (char *)ccb->cam_sim_vid, "SDMMC");
	strncpy( (char *)ccb->cam_hba_vid, (char *)ext->hc_inf.name, sizeof( ccb->cam_hba_vid ) );

	return( CAM_REQ_CMP );
}

// All CCB requests to the SIM are made through this function.
long sdmmc_sim_action( SIM_HBA *hba, CCB *ccbp )
{
	SIM_SDMMC_EXT	*ext;
	CCB_HEADER		*ccb;
	int				status;
...
{
	SIM_HBA					*hba;
	SIM_HBA					*nhba;	// pointer to next hba
	SIM_SDMMC_EXT			*ext;
	int						busno;
	...
	if( sdio_connect( &connect_parm, &sdmmc_ctrl.connection ) != EOK ) {
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  sdio_connect failure", __FUNCTION__ );
		return( CAM_FAILURE );
	}

	if( ( sdmmc_ctrl.cflags & SDMMC_CFLAG_SCAN ) ) {	//auto-detect interfaces
		sdmmc_ctrl.cflags |= SDMMC_CFLAG_ENUMERATING;
		hba = TAILQ_FIRST( &sdmmc_ctrl.hlist );			//take first hba pointer from list

		for( busno = 0; busno < SDMMC_MAX_BUS ; busno++ ) {
			if( hba == NULL && ( hba = sdmmc_alloc_hba( ) ) == NULL ) {	//allocate resources for new nba
				break;
			}
			ext			= (SIM_SDMMC_EXT *)hba->ext;
			inst	 	= &ext->instance;
		...
		for( hba = TAILQ_FIRST( &sdmmc_ctrl.hlist ); hba; hba = nhba ) {
			...
			cam_slogf( _SLOGC_SIM_MMC, _SLOG_DEBUG1, 1, 1, "%s: call sdmmc_attach for inst %p vid 0x%x did 0x%x", __FUNCTION__ , inst,  inst->ident.vid, inst->ident.did);
			if( sdmmc_attach( hba, sdmmc_ctrl.connection, inst ) != CAM_SUCCESS ) {
				cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s:  (specific) sdmmc_detach", __FUNCTION__ );
				sdmmc_detach( hba );
			}
		}
	}
...
int sdmmc_sim_args( char *options )
{
	...
	// XXX here we have only options that come after "sdmmc" module name
//	cam_slogf( _SLOGC_SIM_MMC, _SLOG_DEBUG1, 1, 1, "%s: start; options = %s\n", __FUNCTION__, options? options: "NULL");
...
int sdmmc_set_thread_state( uint32_t *tstate, int state )
{
	pthread_sleepon_lock( );
	*tstate = state;
...
int sdmmc_create_thread( pthread_t *tid, pthread_attr_t *aattr, void *(*func)(void *), void *arg, int priority, uint32_t *tstate, char *name )
{
	pthread_t			_tid;
	...
	if( aattr == NULL ) {
		...
		pthread_attr_setschedparam( &attr, &param );
		pthread_attr_setinheritsched( &attr, PTHREAD_EXPLICIT_SCHED );
		pthread_attr_setstacksize( &attr, SDMMC_STACK_SIZE );
	}
...
//			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 0, 0, "%s:  pthread_setname_np %s", __FUNCTION__, strerror( status ) );
	...
	if( tstate ) {
		...
		pthread_sleepon_unlock( );
		status = *tstate == CAM_TSTATE_INITIALIZED ? EOK : EIO;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sim_sdmmc.c $ $Rev: 777274 $")
#endif
