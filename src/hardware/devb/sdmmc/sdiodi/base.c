...
/* sdio_hc_alloc
 * Allocate memory for new Host Controller data structure
 */
sdio_hc_t *sdio_hc_alloc( )
{
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: start", __FUNCTION__);
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: end hc=%p", __FUNCTION__, hc);
	return( hc );
}
...
int sdio_sg_start( sdio_hc_t *hc, sdio_sge_t *sge, int nsg )
{
	...
	wspc->sga	= (_Uint8t *)sge->sg_address;
...
int sdio_sg_nxt( sdio_hc_t *hc, uint8_t **addr, int *len, int blksz )
{
	...
	if( !wspc->sgc ) {						// advance to next sg entry
		...
		wspc->sga	= (_Uint8t *)wspc->sge->sg_address;
	}
...
// 			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 3, "%s: sleep", __FUNCTION__ );
			...
			if( ( hc->caps & HC_CAP_SLEEP ) ) {
				if( ( dev->caps & DEV_CAP_SLEEP ) ) {
	 			...
	 			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 3, "%s (path %d): sleep hc cnt %lld", __FUNCTION__, hc->path, ++hc->pm_sleep_cnt );
				hc->entry.pm( hc, pm );
			}
			break;
...
int _sdio_set_block_length( sdio_dev_t *dev, int blklen )
{
	...
	int					status;
		...
		// cmd isn't supported in DDR/HC
	if( ( dev->flags & DEV_FLAG_DDR ) && ( dev->caps & DEV_CAP_HC ) ) {
		return( EOK );
	}
...
int _sdio_wait_card_status( sdio_dev_t *dev, uint32_t *rsp, uint32_t mask, uint32_t val, uint32_t msec )
{
	...
	for( msec = max( msec, 1 ); msec; msec-- ) {
		...
		if( hc->cfg.verbosity > 3 ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 3, "%s: rsp 0x%x", __FUNCTION__, rsp[0] );
		}
...
int _sdio_lock_unlock( sdio_dev_t *dev, int op, uint8_t *pwd, int pwd_len )
{
	...
	if( ( status = _sdio_set_block_length( dev, len ) ) == EOK ) {
		...
		sge.sg_count = len; sge.sg_address = (paddr_t)ld;
...
//		if( ( status = pthread_cond_wait( &hc->cond, &hc->mutex ) ) != EOK ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_WARNING, hc->cfg.verbosity, 0, "%s: TIMEOUT (errno %d) CMD %d, flgs 0x%x, arg 0x%x, blks %d, blksz %d", __FUNCTION__, status, cmd->opcode, cmd->flags, cmd->arg, cmd->blks, cmd->blksz );
			break;
		}
	}
...
int sdio_issue_cmd( sdio_dev_t *dev, struct sdio_cmd *cmd, uint64_t tms )
{
	...
	if( hc->cfg.verbosity > 3 ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 3, "%s: CMD %d, flgs 0x%x, arg 0x%x, blks %d, blksz %d, timeout %llums", __FUNCTION__, cmd->opcode, cmd->flags, cmd->arg, cmd->blks, cmd->blksz, tms );
	}
...
int sdio_cmd_cmplt( sdio_hc_t *hc, struct sdio_cmd *cmd, int status )
{
	...
	//	Log all errors regardless of the verbosity setting
	if( status > CS_CMD_CMP || hc->cfg.verbosity > 3 ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s: CMD %d, flgs 0x%x, arg 0x%x, blks %d, blksz %d, status %s (%d)", __FUNCTION__, cmd->opcode, cmd->flags, cmd->arg, cmd->blks, cmd->blksz, name[status], status );
	}
...
int _sdio_bus_error( sdio_dev_t *dev )
{
	...
	//	Log all bus errors regardless of the verbosity level
	hc->bus_errs++;
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s: count = %d", __FUNCTION__, hc->bus_errs);
...
/* _sdio_attach
 * 1. Powers off SD
 * 2. Sets clock and powers on
 * 3. For MMC, runs mmc_ident and turns it to IDLE state
 * 4. Powers off SD again
 */
int _sdio_attach( sdio_hc_t *hc )
{
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: start", __FUNCTION__);
	...
	for( clk = SDIO_CLK_INIT; clk; clk -= 100000 ) {
		...
		sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: power ON with clock %d", __FUNCTION__, hc->clk_init);
		sdio_power( hc, SDIO_PWR_ON );

		dtype = hc->flags & HC_FLAG_DEV_TYPE;

		if( !dtype || ( dtype & HC_FLAG_DEV_SD ) ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: SD device", __FUNCTION__);
			if( ( status = sd_ident( hc ) ) == EOK ) {
				_sdio_pwrmgnt( dev, PM_IDLE );
				dev->flags |= DEV_FLAG_PRESENT;
				dev->dtype = DEV_TYPE_SD;
				sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: sd_ident SUCCESS, dev->flags = %x", __FUNCTION__, dev->flags);
				return( status );
			} else {
				sdio_slogf( _SLOGC_SDIODI, _SLOG_WARNING, hc->cfg.verbosity, 1, "%s: sd_ident FAILED", __FUNCTION__);
			}
		}

		if( !dtype || ( dtype & HC_FLAG_DEV_MMC ) ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: MMC device", __FUNCTION__);
			if( ( status = mmc_ident( hc ) ) == EOK ) {
				_sdio_pwrmgnt( dev, PM_IDLE );
				dev->flags |= DEV_FLAG_PRESENT;
				dev->dtype = DEV_TYPE_MMC;
				sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: mmc_ident SUCCESS, dev->flags = %x", __FUNCTION__, dev->flags);
				return( status );
			} else {
				sdio_slogf( _SLOGC_SDIODI, _SLOG_WARNING, hc->cfg.verbosity, 1, "%s: mmc_ident FAILED", __FUNCTION__);
			}
		}

		sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, hc->cfg.verbosity, 1, "%s: power OFF with clock %d", __FUNCTION__, hc->clk_init);	//FIXME debug
		sdio_power( hc, SDIO_PWR_OFF );
	}
...
int sdio_cd_info( sdio_hc_t *hc )	// Card information
{
	sdio_dev_t			*dev;
	sdio_cid_t			*cid;
	sdio_csd_t			*csd;
	...
	// CID (Card Identification Data Register)
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, cfg->verbosity, 0, "%s Card Identification Data:", dev->dtype == DEV_TYPE_MMC ? "MMC" : "SD" );
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, cfg->verbosity, 0, "  Manufacture ID 0x%x, OEM/Application ID 0x%x, Product Name %s", cid->mid, cid->oid, cid->pnm );
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, cfg->verbosity, 0, "  Product Version 0x%x, Product Serial Number 0x%x, Manufacture Date %02d-%4d",
				cid->prv, cid->psn, cid->month, cid->year );

	// CSD (Card Specific Data Register)
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, cfg->verbosity, 0, "%s Card Specific Data:", dev->dtype == DEV_TYPE_MMC ? "MMC" : "SD" );
	...
	if( dev->dtype == DEV_TYPE_SD ) {
		...
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, cfg->verbosity, 0, "MMC Extended Card Specific Data:" );
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, cfg->verbosity, 0, "CFG:  Timing %s, DTR %d, Bus Width %d bit", name[hc->timing], hc->clk, hc->bus_width );
...
int sdio_cd( sdio_hc_t *hc )	// Card Detect (detect?)
{
	sdio_dev_t				*dev;
	sdio_funcs_t			*funcs;
	sdio_hc_entry_t			*hce;
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: start, cd state = 0x%08x", __FUNCTION__, cd);

	if( ( dev->flags & DEV_FLAG_MEDIA_CHANGE ) || !( cd & CD_INS ) ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: media change", __FUNCTION__);

		atomic_clr( &dev->flags, ( DEV_FLAG_MEDIA_CHANGE | DEV_FLAG_INVALID_CARD | DEV_FLAG_WRITE_PROTECT ) );

		if( ( dev->flags & DEV_FLAG_PRESENT ) ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s: removal path %d, cd state 0x%x", __FUNCTION__, hc->path, cd );
	...
	if( ( cd & CD_INS ) ) {	// Card inserted
		sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 0, "%s: card inserted", __FUNCTION__);
		if( !( dev->flags & ( DEV_FLAG_PRESENT | DEV_FLAG_INVALID_CARD ) ) ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s: insertion path %d, cd state 0x%x", __FUNCTION__, hc->path, cd );
			if( _sdio_attach( hc ) == EOK ) {

				sdio_cd_info( hc );	// Log information about Card
...
int sdio_timer_settime( int tid, uint32_t sec, uint32_t nsec, int repeat )
{
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, 10, 0, "%s: start", __FUNCTION__);
...
void *sdio_cd_thread( void *hdl )
{
	...
	if( ( sc->cd_chid = ChannelCreate( _NTO_CHF_DISCONNECT | _NTO_CHF_UNBLOCK ) ) == -1 ||
	...
	while( 1 ) {
		...
		sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: got pulse code %d", __FUNCTION__, pulse.code);

		switch( pulse.code ) {
			case SDIO_EV_TIMER:	// code 0
...
void *sdio_hc_thread( void *hdl )
{
	...
	if( pthread_condattr_init( &attr ) ||
			...
			( hc->hc_chid = ChannelCreate( _NTO_CHF_DISCONNECT | _NTO_CHF_UNBLOCK ) ) == -1 ||
...
int sdio_options( sdio_hc_t *hc, char *options )
{
	...
	while( options && *options != '\0' && status == EOK ) {
		if( ( opt = getsubopt( &options, opts, &value ) ) == -1 ) {
			...
			//	Try to be kind and give user a hint about it.
		...
		switch( opt ) {
			...
			case 11:		// timing
				...
				while( value != NULL ) {
					...
					else if( !strcmp( value, "sdr" ) )		*cap |= ( HC_CAP_SDR12 | HC_CAP_SDR25 | HC_CAP_SDR50 | HC_CAP_SDR104 );
...
/*
 * sdio_hc_lookup
 * Check if this device is Host Controller we support
 */
sdio_product_t *sdio_hc_lookup( int vid, int did, int class, char *name )
{
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_INFO, sdio_ctrl.verbosity, 1, "%s: vid = 0x%x did = 0x%x class=0x%x", __FUNCTION__, vid, did, class);
...
int sdio_hc_init( sdio_hc_t *hc )
{
	...
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: cfg->vid = 0x%x cfg->did=0x%x", __FUNCTION__, cfg->vid, cfg->did);
	...
	if( ( prod = sdio_hc_lookup( cfg->vid, cfg->did, cfg->class, cfg->name ) ) ) {
		if( ( status = sdio_create_thread( &hc->hc_tid, NULL, sdio_hc_thread, hc, hc->priority, &hc->state, "sdio_hc_thread" ) ) == EOK ) {
			if( ( status = prod->init( hc ) ) == EOK ) {
				if( cfg->name[0] == NULL ) {
					strcpy( cfg->name, prod->name );
				}
				hc->flags |= HC_FLAG_INITIALIZED;
				sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: hc init success %s", __FUNCTION__, prod->name );
				return( EOK );
			}
			else {
				sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, sdio_ctrl.verbosity, 1, "%s: hc init failure %s", __FUNCTION__, prod->name );
			}
		}
		else {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, sdio_ctrl.verbosity, 1, "%s: sdio_hc_thread creation failure", __FUNCTION__ );
		}
	}
	else {
		if( cfg->name[0] != '\0' ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, sdio_ctrl.verbosity, 1, "%s sdio_hc_lookup failure %s", __FUNCTION__, cfg->name );
		}
		else {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, sdio_ctrl.verbosity, 1, "%s sdio_hc_lookup failure vid %x, did %x", __FUNCTION__, cfg->vid, cfg->did );
		}
		status = ENODEV;
	}
...
{
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: sc %p", __FUNCTION__, sc );
...
/* sdio_init
 * 1. Creates sdio_cd_thread thread
 */
...
/* _sdio_connect
 * 1. Runs sdio_init function
 * 2. ... (TODO describe next steps)
 */
int _sdio_connect( sdio_connect_parm_t *parm, struct sdio_connection **connection )
{
...
#ifdef SDIO_PCI_SUPPORT
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: calling sdio_pci_init...", __FUNCTION__ );
	sdio_pci_init( );
#endif
	...
	while( ( margs = sdio_module_args( "sdio", parm->argc, parm->argv, occurence++ ) ) ) {
		...
		if( !( sdio_ctrl.flags & SDIO_CFLAG_SCAN ) ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: calling sdio_hc_alloc...", __FUNCTION__ );
			if( ( hc = sdio_hc_alloc( ) ) ) {
				sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: calling sdio_options...", __FUNCTION__ );
		...
		if( status ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, sdio_ctrl.verbosity, 1, "%s: failed status", __FUNCTION__ );
			sdio_dinit( &sdio_ctrl );
			return( status );
		}
	}
	...
	if( TAILQ_FIRST( &sdio_ctrl.hlist ) == NULL || ( sdio_ctrl.flags & SDIO_CFLAG_SCAN ) ) {
#ifdef SDIO_PCI_SUPPORT
		sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: calling sdio_pci_scan...", __FUNCTION__ );
		sdio_pci_scan( );
#endif
		...
		for( hc = TAILQ_FIRST( &sdio_ctrl.hlist ); hc; hc = TAILQ_NEXT( hc, hlink ) ) {
			...
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR,  sdio_ctrl.verbosity, 1, "%s: another sdio_dinit() call...", __FUNCTION__);
			sdio_dinit( &sdio_ctrl );
			return( status );
		}
	}

	// Now we have filled in Host Controllers list, starting Host Controllers initialization.
	for( hc = TAILQ_FIRST( &sdio_ctrl.hlist ), tmr = 0; hc; hc = TAILQ_NEXT( hc, hlink ) ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: calling sdio_hc_init()", __FUNCTION__);
		if( ( status = sdio_hc_init( hc ) ) != EOK ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s: calling sdio_dinit()", __FUNCTION__);
			sdio_dinit( &sdio_ctrl );
			return( status );
		}

        /* SWD fix/workaround
         * We'll always call sdio_cd() explicitly. Seems there is no point to send pulse at this moment because in fact
         * receiving thread will call sdio_cd() too, but in this case we get race conditions between _sdio_attach() function
         * which sets flag (dev->flags |= DEV_FLAG_PRESENT) and sdio_attach() function which checks 'hba->device.flags'
         * (hc->device.flags & DEV_FLAG_PRESENT)
        */
//      if( ( hc->caps & HC_CAP_CD_INTR ) ) {
		...
		/* If Card Detect interrupts are not supported and Slot is not embedded,
		 * we have to detect cards using timer
		 */
		if( !( hc->caps & HC_CAP_CD_INTR ) && !( hc->caps & HC_CAP_SLOT_TYPE_EMBEDDED ) ) {
			tmr = SDIO_TRUE;
		}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/base.c $ $Rev: 788126 $")
#endif
