...
/* sdio_pci_config
 * Saving Host Controller configuration based on PCI device configuration
 *
 */
int sdio_pci_config( sdio_hc_t *hc, sdio_product_t *prod, struct pci_dev_info *pdev )
{
	sdio_hc_cfg_t		*cfg;	// Host Controller configuration
	int					idx;

	// Copying Host Controller configuration from PCI device configuration structure to hc->cfg structure
	cfg							= &hc->cfg;
...
/* sdio_pci_device
 * 1. Check Host Controller PCI device
 * 2. Call pci_attach_device() function
 * 3. Save its configuration to hc->cfg structure
 */
int sdio_pci_device( sdio_hc_t *hc )
{
	...
	sdio_slogf(_SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: calling sdio_hc_lookup...", __FUNCTION__);

	if( ( prod = sdio_hc_lookup( cfg->vid, cfg->did, cfg->class, "" ) ) == NULL ) {
		return( ENODEV );
	}

	sdio_slogf(_SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: filling pdev structure...", __FUNCTION__);
	...
	if( ( phdl = pci_attach_device( NULL, aflgs, cfg->idx, &pdev ) ) == NULL ) {
		sdio_slogf(_SLOGC_SDIODI, _SLOG_ERROR, sdio_ctrl.verbosity, 1, "%s: pci_attach_device() FAILED.", __FUNCTION__);
		return( ENODEV );
	}

	hc->phdl		= phdl;

	/* Save config to hc->cfg */
...
/* sdio_pci_scan
 * Scan all PCI buses and devices, find Host Controllers using sdio_hc_lookup() function
 */
int sdio_pci_scan( )
{
	sdio_hc_t			*hc;
	...
	for( bus = 0; bus <= lastbus; bus++ ) {
		for( dev = 0; dev < 32; dev++ ) {
			for( func = 0, nfuncs = 8; func < nfuncs; func++ ) {
				...
				sdio_slogf(_SLOGC_SDIODI, _SLOG_DEBUG1, sdio_ctrl.verbosity, 1, "%s: check if we support this device using sdio_hc_lookup()...", __FUNCTION__);
...
//					Allocate memory for new Host Controller data structure
					...
					// Get Host Controller device PCI configuration
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/pci.c $ $Rev: 781990 $")
#endif
