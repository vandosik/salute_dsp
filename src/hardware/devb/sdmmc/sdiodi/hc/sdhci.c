/*
 * $QNXLicenseC:
 * Copyright 2007, 2008, 2015, QNX Software Systems.
 *
...
static int sdhci_reset( sdio_hc_t *hc, uint32_t rst )
{
...
#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s:  rst %x", __FUNCTION__, rst );
#endif
	...
	if( status ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s:  timeout", __FUNCTION__ );
	}

#if defined (VARIANT_tegra3)
	uint32_t regval;
	// Enable SD3.0 support on the controller.
	regval = sdhci_in32(base + SDHCI_VENDOR_MISC);
	regval |= SDHCI_CAP_SD3_0;
	sdhci_out32(base + SDHCI_VENDOR_MISC, regval);

	// Apply tap value
	regval = sdhci_in32(base + SDHCI_VENDOR_CLK_CTRL);
	regval &= ~SDHCI_TAP_VAL_MASK;
	regval |= SDHCI_TAP_VAL;
	sdhci_out32(base + SDHCI_VENDOR_CLK_CTRL, regval);
#endif
	return( status );
}
...
#ifdef SDHCI_DEBUG
// Interrupt status and Interrupt enable Registers
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s:  is 0x%x, ie 0x%x", __FUNCTION__, sdhci_in32( base + SDHCI_IS ), sdhci_in32( base + SDHCI_IE ) );
...
#ifdef SDHCI_DEBUG
//	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: ", __FUNCTION__ );
#endif
...
/* sdhci_pwr
 * 1. If vdd or hc->vdd is 0 - performs 3 resets: mmci_cmd, mmci_data and all
 */
static int sdhci_pwr( sdio_hc_t *hc, int vdd )
{
	sdhci_hc_t		*sdhc;
...
#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: vdd 0x%x", __FUNCTION__, vdd );
#endif
...
#ifdef SDHCI_DEBUG
//	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s: zero vdd, reset core...", __FUNCTION__ );
#endif
		sdhci_reset( hc, SDHCI_SYSCTL_SRC );
		sdhci_reset( hc, SDHCI_SYSCTL_SRD );
...
//		delay( 10 );
		...
		sdhci_out32( base + SDHCI_HCTL, 0 );	// clear Host Control register
	}
...
#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: end", __FUNCTION__ );
#endif
...
#ifdef VARIANT_mcom
	sdhci_waitmask( hc, SDHCI_SYSCTL, SDHCI_SYSCTL_SRC | SDHCI_SYSCTL_SRD, ~(SDHCI_SYSCTL_SRC | SDHCI_SYSCTL_SRD), 10000 );
#endif
	hc->clk = clk;
...
#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: signal_voltage %d", __FUNCTION__, signal_voltage );
#endif
...
#if defined (VARIANT_tegra3) // bypass this check for eMMC
		if( !( sdhci_in32( base + SDHCI_PSTATE ) & SDHCI_PSTATE_DLSL_MSK ) || hc->flags & HC_FLAG_DEV_MMC ) {
#else
		if( !( sdhci_in32( base + SDHCI_PSTATE ) & SDHCI_PSTATE_DLSL_MSK ) ) {
#endif
			sdhci_out16( base + SDHCI_HCTL2, sdhci_in16( base + SDHCI_HCTL2 ) | SDHCI_HCTL2_SIG_1_8V );
...
#else
				if( ( sdhci_in32( base + SDHCI_PSTATE ) & SDHCI_PSTATE_DLSL_MSK ) == SDHCI_PSTATE_DLSL_MSK ) {
#endif
...
static int sdhci_tune( sdio_hc_t *hc, int op )
{
	...
	if( status || ( hctl2 & SDHCI_HCTL2_EXEC_TUNING ) || !( hctl2 & SDHCI_HCTL2_TUNED_CLK ) ) {
...
int sdhci_dinit( sdio_hc_t *hc )
{
...
#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, 1, 1, "%s: start", __FUNCTION__ );
#endif
	...
	if( sdhc->adma ) {
		munmap( sdhc->adma, sizeof( sdhci_adma32_t ) * ADMA_DESC_MAX );
	}

	free( sdhc );

#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s: end", __FUNCTION__ );
#endif
...
int sdhci_init( sdio_hc_t *hc )
{
...
#ifdef SDHCI_DEBUG
	sdio_slogf( _SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: start", __FUNCTION__ );
#endif
	...
	if( ( base = sdhc->base = (uintptr_t)mmap_device_memory( NULL, cfg->base_addr_size[0],
		...
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s: SDHCI base mmap_device_memory (0x%llx) %s", __FUNCTION__, cfg->base_addr[0], strerror( errno ) );
		sdhci_dinit( hc );
		return( errno );
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/hc/sdhci.c $ $Rev: 790216 $")
#endif
