...
#define	CCC_MANDATORY   ( CCC_BASIC | CCC_BLOCK_READ | CCC_ERASE | CCC_APP_SPEC )
	_Uint16t	ccc;
...
struct _sdio_sge {
#if 1
    paddr_t			sg_address;				// Scatter/Gather address
    _Uint32t		sg_count;				// Scatter/Gather count
#else
    paddr64_t		sg_address;				// Scatter/Gather address
    _Uint32t		sg_count;				// Scatter/Gather count
    _Uint32t		rsvd;
#endif
};
...
#define	SCF_DATA_PHYS		(1 << 9)	// data read
#define	SCF_DIR_IN			(1 << 10)	// data read
#define	SCF_DIR_OUT			(1 << 11)	// data write
#define	SCF_DATA_MSK		(SCF_DIR_IN | SCF_DIR_OUT)
#define	SCF_APP_CMD			(1 << 12)	// app command (cmd 55)
#define	SCF_SBC				(1 << 13)	// auto issue set block count (cmd 23)
#define	SCF_AC12			(1 << 14)	// auto cmd 12
#define	SCF_MULTIBLK		(1 << 15)
#define	SCF_WAIT_DRDY		(1 << 16)	// wait ready for data
...
#define DEV_CAP_ASSD		(1 << 11)	// ASSD (Advanced Security SD Extension)
...
#define	HC_CAP_DMA					(1 << 2)	// supports DMA
#define	HC_CAP_BW4					(1 << 3)	// 4 bit bus supported
#define	HC_CAP_BW8					(1 << 4)	// 8 bit bus supported
#define	HC_CAP_ACMD12				(1 << 5)	// auto stop cmd(12) supported
#define	HC_CAP_ACMD23				(1 << 6)	// auto set block count cmd(23) supported
#define HC_CAP_SLEEP				(1 << 7)

#define	HC_CAP_HS					(1 << 8)	// High speed device supported
#define	HC_CAP_SDR12				(1 << 9)
#define	HC_CAP_SDR25				(1 << 10)
#define	HC_CAP_SDR50				(1 << 11)
#define	HC_CAP_SDR104				(1 << 12)
#define	HC_CAP_DDR50				(1 << 13)	// Dual Data Rate supported
#define HC_CAP_HS200				(1 << 14)
	_Uint64t		caps;
	_Uint32t		version;
	_Uint32t		sg_max;
...
extern int				sdio_flush_cache( struct sdio_device *dev );
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/include/sdiodi.h $ $Rev: 738345 $")
#endif
