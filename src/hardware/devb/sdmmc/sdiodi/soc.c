...
int sdio_soc_config( sdio_hc_t *hc, int idx, unsigned offset )
{
	...
	if( cfg->name[0] == '\0' ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 1, "%s:  skipping SOC %d, base 0x%llx", __FUNCTION__, idx, cfg->base_addr[0] );
		return( ENODEV );
	}
...
int sdio_soc_device( sdio_hc_t *hc )
{
	...
	status	= EOK;
#ifndef VARIANT_mcom
    //Don't like this place, the name will be owerwritten anyway
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/soc.c $ $Rev: 711024 $")
#endif
