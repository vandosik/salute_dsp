...
int sdio_flush_cache( struct sdio_device *device )
{
	int				status;
					...
					void *client_hdl )
{
	...
	for( hc = TAILQ_FIRST( &sdio_ctrl.hlist ); hc; hc = TAILQ_NEXT( hc, hlink ) ) {
		sdio_slogf(_SLOGC_SDIODI, _SLOG_DEBUG1, hc->cfg.verbosity, 1, "%s: hc = %p, hc->path = %d, instance->path = %d, device.flags = 0x%08x",
				__FUNCTION__, hc, hc->path, instance->path, hc->device.flags);
...
//	sdio_slogf(_SLOGC_SDIODI, _SLOG_DEBUG1, 10, 1, "%s: hc found = %p", __FUNCTION__, hc);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/card.c $ $Rev: 728112 $")
#endif
