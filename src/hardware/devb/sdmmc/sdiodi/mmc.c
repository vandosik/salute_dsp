...
sdio_device_errata_t mmc_errata[] = {
	...
	{ "MAG2GA", 0x15, 0x0, 0x0, 0x6, DEV_ERRATA_ACMD12 | DEV_ERRATA_DISCARD_SUP, 0 },
		// fw added support for DISCARD
	{ "MAG2GA", 0x15, 0x0, 0x0, DEV_ERRATA_WILDCARD, DEV_ERRATA_DISCARD_SUP, 0 },
		// fw bug where device will discard data when AC12 is used
	{ "MBG4GA", 0x15, 0x0, 0x0, 0x6, DEV_ERRATA_ACMD12 | DEV_ERRATA_DISCARD_SUP, 0 },
		// fw added support for DISCARD
	{ "MBG4GA", 0x15, 0x0, 0x0, DEV_ERRATA_WILDCARD, DEV_ERRATA_DISCARD_SUP, 0 },
	{ "MCG8GA", 0x15, 0x0, 0x0, DEV_ERRATA_WILDCARD, DEV_ERRATA_DISCARD_SUP, 0 },

	// Sandisk devices
	{ "SEM08G", 0x45, 0x0, 0x0, DEV_ERRATA_WILDCARD, DEV_ERRATA_DISCARD_SUP, 0 },
...
int mmc_parse_cid( sdio_dev_t *dev, sdio_cid_t *cid, uint32_t *rsp )
{
	...
	if( ( errata = sdio_device_errata( dev, mmc_errata ) ) ) {
...
int mmc_parse_ext_csd( sdio_dev_t *dev, sdio_ecsd_t *ecsd, uint8_t *raw_ecsd )
{
	...
	if( ( ( hc->caps & HC_CAP_HS200 ) && ( ecsd->card_type & ECSD_CARD_TYPE_200MHZ ) ) &&
		( ( ( hc->caps & HC_CAP_SV_1_8V ) && ( ecsd->card_type & ECSD_CARD_TYPE_SDR_1_8V ) ) ||
				( ( hc->caps & HC_CAP_SV_1_2V ) && ( ecsd->card_type & ECSD_CARD_TYPE_SDR_1_2V ) ) ) ) {
		dev->caps			|= DEV_CAP_HS200 | DEV_CAP_HS;
		ecsd->dtr_max_hs	= DTR_MAX_HS200;
	}
	else if( ( hc->caps & HC_CAP_HS ) && ( ecsd->card_type & ECSD_CARD_TYPE_52MHZ ) ) {
		dev->caps			|= DEV_CAP_HS;
...
int mmc_send_ext_csd( sdio_dev_t *dev, uint8_t *csd )
{
	...
	sge.sg_count = MMC_EXT_CSD_SIZE; sge.sg_address = (paddr_t)ecsd;
...
int mmc_init_hs200( sdio_hc_t *hc, int bus_width )
{
    ...
    dev       = &hc->device;
	ecsd	  = &dev->ecsd;
    status    = EINVAL; // start with EINVAL, because signal voltage must be set either to 1.2 or 1.8 Volt

    if( ( ecsd->card_type & ECSD_CARD_TYPE_SDR_1_2V ) && ( hc->caps & HC_CAP_SV_1_2V ) ) {
    ...
    if( status && ( ecsd->card_type & ECSD_CARD_TYPE_SDR_1_8V ) && ( hc->caps & HC_CAP_SV_1_8V ) ) {
    ...
    // if signal voltage switch to 1.2 or 1.8 volt failed, exit here. HS200 requires 1.2 or 1.8 volt
    if( status ) {
        return( status );
    }
    ...
    if( ( status = mmc_switch( dev, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BUS_WIDTH, bus_mode, SDIO_TIME_DEFAULT ) ) ) {
        sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 0, "%s: switch ext_csd_bus_width", __FUNCTION__ );
        return( status );
    }
...
int mmc_init_ddr( sdio_hc_t *hc, int bus_width )
{
    ...
    if( ( status = mmc_switch( dev, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BUS_WIDTH, bus_mode, SDIO_TIME_DEFAULT ) ) ) {
        sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 1, "%s: switch ECSD_BUS_WIDTH %x", __FUNCTION__, bus_mode );
        return( status );
    }
...
int mmc_init_bus( sdio_hc_t *hc )
{
    ...
    if( ( dev->caps & DEV_CAP_HS200 ) && bus_width >= BUS_WIDTH_4 ) {
        mmc_init_hs200( hc, bus_width );
    }

        // enable HS if supported
    if( !( dev->flags & DEV_FLAG_HS200 ) && ( dev->caps & DEV_CAP_HS ) ) {
        mmc_init_hs( hc );
    }

    if( ( dev->flags & DEV_FLAG_HS ) ) {
        if( bus_width >= BUS_WIDTH_4 ) {
            if( ( dev->caps & DEV_CAP_DDR50 ) ) {        // enable DDR if supported
                mmc_init_ddr( hc, bus_width );
            }

            if( !( dev->flags & DEV_FLAG_DDR ) ) {    // enable 4/8 bit mode
                if( mmc_switch( dev, MMC_SWITCH_CMDSET_DFLT, MMC_SWITCH_MODE_WRITE, ECSD_BUS_WIDTH, bus_width / 4, SDIO_TIME_DEFAULT ) ) {
    ...
    if( !( dev->flags & ( DEV_FLAG_HS200 | DEV_FLAG_HS ) ) ) {
        sdio_timing( hc, TIMING_LS );
        sdio_clock( hc, dev->csd.dtr_max );
    }

    if( ( status = _sdio_set_block_length( dev, SDIO_DFLT_BLKSZ ) ) != EOK ) {
        sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 0, "%s: sdio_set_block_lenght", __FUNCTION__ );
        return( status );
    }
...
int mmc_bus_error( sdio_dev_t *dev )
{
	...
	if( ( dev->flags & DEV_FLAG_HS200 ) ) {
		//	Disable HS200 because it doesn't seem to be working well enough.
		//	Issue slog message regardless of the verbosity settings.
		sdio_slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: disabling HS200", __FUNCTION__);
		...
		//	Issue slog message regardless of the verbosity settings.
		sdio_slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s: disabling DDR50", __FUNCTION__);
		dev->caps	&= ~DEV_CAP_DDR50;
		dev->flags	&= ~DEV_FLAG_DDR;
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/mmc.c $ $Rev: 745369 $")
#endif
