...
sdio_device_errata_t sd_errata[] = {
	...
	{ "SD0", 0x27, DEV_ERRATA_WILDCARD, 0, DEV_ERRATA_WILDCARD, 0, 50 },
...
int sd_parse_csd( sdio_dev_t *dev, sdio_csd_t *csd, uint32_t *rsp )
{
	...
	//	Some faulty SD-cards return invalid CSD data.
	//
	//	The strange CSD data can cause all sorts of problems later.
	//	Better to detect the problem in here, and reject the card.
	//
	//	The standard requires some command classes to always be supported.
	//	At least classes 0, 2, 5, and 8 should be present.
	//	If a mandatory class is missing, the CSD data is deemed invalid,
	//	and the card is assumed to be unusable.
	//
	if( ( csd->ccc & CCC_MANDATORY ) != CCC_MANDATORY ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, 1, 0, "%s: invalid CCC 0x%04x", __FUNCTION__, csd->ccc );
		return( EINVAL );
	}
...
int sd_switch( sdio_dev_t *dev, int mode, int grp, uint8_t val, uint8_t *switch_status )
{
	...
	sge.sg_count = SD_SF_STATUS_SIZE; sge.sg_address = (paddr_t)sbuf;
...
int sd_app_send_scr( sdio_dev_t *dev, uint32_t *scr )
{
	...
	sge.sg_count = SD_SCR_SIZE; sge.sg_address = (paddr_t)sbuf;
...
int sd_app_sd_status( sdio_dev_t *dev, uint32_t *sds )
{
	...
	sge.sg_count = SD_STATUS_SIZE; sge.sg_address = (paddr_t)sbuf;
...
int sd_app_send_op_cond( sdio_hc_t *hc, uint32_t ocr, uint32_t *rocr )
{
	...
	if( hc->cfg.verbosity > 3 ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 0, "%s: retries %d", __FUNCTION__, OCR_BUSY_RETRIES - retry );
	}
...
int sd_init_uhs( sdio_dev_t *dev )
{
	...
	if( ( sdio_tune( hc, SD_SEND_TUNING_BLOCK ) ) ) {
		sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 0, "%s: tuning failure", __FUNCTION__ );
	}
...
int sd_init_bus( sdio_hc_t *hc )
{
	...
	if( hc->signal_voltage == SIGNAL_VOLTAGE_1_8 ) {	// UHS card
		if( ( status = sd_init_uhs( dev ) ) != EOK ) {
				// knock down UHS caps
			hc->caps &= ~( HC_CAP_SDR104 | HC_CAP_SDR50 | HC_CAP_SDR25 | HC_CAP_SDR12 );
			return( status );
		}
	}
	else {
...
int sd_init_device( sdio_hc_t *hc, uint32_t ocr, int flgs )
{
	...
	if( !( dev->flags & DEV_FLAG_SIG_ERR ) && HC_CAP_UHS( hc->caps ) ) {
...
int sd_bus_error( sdio_dev_t *dev )
{
	...
	if( ( dev->flags & DEV_FLAG_UHS ) ) {
		...
		if( !swcaps->bus_mode ) {	// reset to get out of 1.8V
			dev->caps &= ~DEV_CAP_UHS;
			return( _sdio_reset( dev ) );
		}
	}
	else if( ( dev->flags & DEV_FLAG_HS ) ) {
		dev->flags	&= ~DEV_FLAG_HS;
		if( swcaps->dtr_max_hs == DTR_MAX_HS26 ) {
			dev->caps &= ~DEV_CAP_HS;
		}
		else {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.5.0/trunk/hardware/devb/sdmmc/sdiodi/sd.c $ $Rev: 778275 $")
#endif
